...
export const ReceiptReviewModal = ({
  ...
  const [editForm, setEditForm] = useState({ nome: "", quantidade: "", precoTotal: "", categoria: "" });
    ...
    setItems(items.map((item) =>
      item.id === editingId
        ? {
            ...
            precoTotal: parseFloat(editForm.precoTotal) || 0,
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-2">
          {items.length === 0 ? (
            <p className="text-center text-muted-foreground py-8">
              {locale === "pt-PT" ? "Nenhum item para adicionar" : "No items to add"}
            </p>
          ) : (
            items.map((item) => (
              <div
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div className="space-y-1">
                        ...
                        <Input
                          type="number"
                          step="0.01"
                          value={editForm.precoTotal}
                          onChange={(e) => setEditForm({ ...editForm, precoTotal: e.target.value })}
                          className="h-8"
                        />
                      </div>
                    </div>
